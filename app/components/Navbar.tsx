...
import { FaBars, FaTimes, FaUser, FaCog, FaSignOutAlt, FaBell, FaMoon, FaSun } from 'react-icons/fa';
...
const Navbar = ({ currentPage = 'home', onNavigate, onLogout }: NavbarProps) => {
  ...
  return (
    <div className="relative mx-4 mt-4">
      <nav className="bg-white border-2 border-slate-200 px-4 md:px-8 py-4 rounded-2xl flex items-center justify-between shadow-sm">
        ...
        <div className="flex items-center space-x-4">
          ...
          <button
            ...
            className="md:hidden text-slate-700 cursor-pointer hover:text-slate-900 transition-colors text-lg"
      ...
      {openMobileMenu && (
        <div className="md:hidden absolute left-0 right-0 mt-2 bg-white border-2 border-slate-200 rounded-2xl shadow-xl z-40 p-3">
          <div className="flex flex-col gap-2">
            <button
              onClick={() => handleMobileNavigate('home')}
              className={`text-left px-4 py-3 rounded-xl font-semibold transition-colors ${currentPage === 'home' ? 'bg-slate-100 text-slate-900' : 'text-slate-700 hover:bg-slate-50'}`}
            >
              Home
            </button>
            <button
              onClick={() => handleMobileNavigate('services')}
              className={`text-left px-4 py-3 rounded-xl font-semibold transition-colors ${currentPage === 'services' ? 'bg-slate-100 text-slate-900' : 'text-slate-700 hover:bg-slate-50'}`}
            >
              Services
            </button>
            <button
              onClick={() => handleMobileNavigate('bookings')}
              className={`text-left px-4 py-3 rounded-xl font-semibold transition-colors ${currentPage === 'bookings' ? 'bg-slate-100 text-slate-900' : 'text-slate-700 hover:bg-slate-50'}`}
            >
              My Bookings
            </button>
            <button
              onClick={() => handleMobileNavigate('myaccount')}
              className={`text-left px-4 py-3 rounded-xl font-semibold transition-colors ${currentPage === 'myaccount' ? 'bg-slate-100 text-slate-900' : 'text-slate-700 hover:bg-slate-50'}`}
            >
              My Account
            </button>
            <button
              ...
              className="text-left px-4 py-3 rounded-xl font-semibold text-slate-700 hover:bg-slate-50 transition-colors"
            >
              AI Chat
            </button>
          </div>
